...
namespace Yvtu.Infra.Data
{
    public class PartnerActivityRepo : IPartnerActivityRepo
    {
        ...
        public async Task<OpertionResult> CreateAsync(PartnerActivity partnerActivity)
        {
            ...
            try
            {
                ...
                var parameters = new List<OracleParameter> {
                 ...
                 new OracleParameter{ ParameterName = "v_queryduration",OracleDbType = OracleDbType.Varchar2,  Value = partnerActivity.MaxQueryDuration },
                 new OracleParameter{ ParameterName = "v_act_scope",OracleDbType = OracleDbType.Varchar2,  Value = partnerActivity.Scope },
        ...
        {
            var actDataTable = this.db.GetData("Select * from v_partner_activity  order by act_type,act_id", null);
            ...
            if (actDataTable != null)
            {
                foreach (DataRow row in actDataTable.Rows)
                {
                    ...
                    partAct.MaxQueryDuration = row["queryduration"] == DBNull.Value ? string.Empty : row["queryduration"].ToString();
                    partAct.Scope = row["act_scope"] == DBNull.Value ? string.Empty : row["act_scope"].ToString();
